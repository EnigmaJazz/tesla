...
try {
    ...
    if (idx > master.length) { 
        ...
        for (let i = idx; i <= master.length; i++) {
            ...
            let activeBaseForState = getBase(state.time);
            let distStateToBase = getDist(parseFloat(state.loc.split(",")[0]), parseFloat(state.loc.split(",")[1]), parseFloat(activeBaseForState.coords.split(",")[0]), parseFloat(activeBaseForState.coords.split(",")[1]));
            let isPrevBase = (distStateToBase < 150);
            if (i === idx && currentlyAtBase) isPrevBase = true;
            ...
            if (ev.isDropin) {
                ...
                if (nextStrict) {
                    let timeGapStrict = Math.abs(evStart - forceSeconds(nextStrict.start));
                    let sLocP = state.loc.split(",");
                    let dLocP = evCoords.split(",");
                    let nLocP = nextStrict.coords.split(",");
                    
                    let distDirect = getDist(parseFloat(sLocP[0]), parseFloat(sLocP[1]), parseFloat(nLocP[0]), parseFloat(nLocP[1]));
                    let distDetour = getDist(parseFloat(sLocP[0]), parseFloat(sLocP[1]), parseFloat(dLocP[0]), parseFloat(dLocP[1])) + getDist(parseFloat(dLocP[0]), parseFloat(dLocP[1]), parseFloat(nLocP[0]), parseFloat(nLocP[1]));
                    
                    let detourDiff = distDetour - distDirect;
                    if (timeGapStrict < 21600 || detourDiff < 10000) {
                ...
                if (isAttachedDropin) {
                    if (isPrevBase && nextStrict) {
                        ...
                        let distToNext = getDist(parseFloat(eLocP[0]), parseFloat(eLocP[1]), parseFloat(nC[0]), parseFloat(nC[1]));
