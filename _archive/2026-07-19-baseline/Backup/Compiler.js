    ...
    for (let m = 0; m < masterArr.length; m++) {
        ...
        if (safeId === evId) {
            ...
            if (apiType === "DEPART" && mode !== "TRANSIT") {
                ...
                let targetBufferSecs = isDepartEventLateCheck ? 0 : (arrMatch ? parseInt(arrMatch[1], 10) : defArrMins) * 60;
                
                let idealDeparture;
                let targetArrival;
                
                if (isDepartEventLateCheck) {
                ...
                if (itinerary.length > 0) {
                    ...
                    for (let p = 0; p < masterArr.length; p++) {
                        ...
                        if (pId === prevLeg.targetEventId) {
                            ...
                            if (isPrevDropin) {
                                let dropinDur = masterArr[p].duration || 0;
                                let adHocStopsRaw = masterArr[p].desc || "";
                                let adHocMatch;
                                let adHocSecs = 0;
                                let stopRegex = /#stop:(\d+)/gi;
                                while ((adHocMatch = stopRegex.exec(adHocStopsRaw)) !== null) {
                                    adHocSecs += (parseInt(adHocMatch[1], 10) * 60);
                                }
                                hardFloor = prevArr + dropinDur + adHocSecs;
                            } else {
                ...
                if (isDropin && !isPrevBase) {
                    actualDeparture = hardFloor;
                } else {
                    if (isDropin) idealDeparture = apiUnix;
            ...
            let delta = evStartSecs - (isDepartEventLateCheck ? actualDeparture : actualArrival); 
            if (delta >= 0) {
                actualBuffer = Math.floor(delta / 60);
                if (isDepartEventLateCheck && actualBuffer === 0) {
                    let depM = targetDesc.match(/(?:#dep:|#leave:)(\d+)/i);
                    let defDepMins = parseInt(global('Departure_Buffer_Mins'), 10) || 5;
                    actualBuffer = depM ? parseInt(depM[1], 10) : defDepMins;
                }
            } else {
                actualLate = Math.ceil(Math.abs(delta) / 60);
            }
