...
try {
    ...
    let tempRaw = global('Temp_Route_Cache') || "";
    ...
    if (tempRaw.length > 5) {
        ...
        setGlobal('Temp_Route_Cache', keepTemp.join("|"));
    ...
    while (local('ce_title' + l)) {
        ...
        if (/(leave|holiday|vacation|oof|out of office)/i.test((lt + " " + ld).toLowerCase())) {
    ...
    for (let a = 0; a < arraysToPrune.length; a++) {
        ...
        for (let k = 0; k < items.length; k++) {
            ...
            if (arrName === "Depart_Memory") {
                ...
                // Keep memory if it departs in the future, or happened within the last 4 hours
            ...
            // [SURGICAL UPGRADE: Scans backwards to safely find the Base36 timestamp even if _IN/_OUT is present]
            let idParts = baseKey.split("_");
            let eventStartUnix = 0;
            
            for (let p = idParts.length - 1; p >= 0; p--) {
                let parsed = parseInt(idParts[p], 36);
                // Validate it is a plausible recent/future epoch timestamp
            ...
            if (eventStartUnix > 0) {
                // Keep overrides indefinitely into the future, sweep away 24 hours after they finish
