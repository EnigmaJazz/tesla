...
try {
    var masterStr = "";
    try { masterStr = readFile("Tasker/Tesla/Data/Itin_Master.json") || "[]"; } catch(e) { masterStr = "[]"; }
    var master = [];
    try { master = JSON.parse(masterStr); } catch(e) { master = []; }
    ...
    // ==========================================
    // SECTION 1: COMPACT UI MARKDOWN COMPILER
    // ==========================================
    if (showTrips) {
        ...
        for (var i = 0; i < total_legs; i++) { 
            ...
            // [SURGICAL UPGRADE: Strip Auto-Proxy prefixes for a cleaner UI]
            ...
            var tripLine = prefix + dayPrefix + time + " " + modeStr + warn + holdStr + " to " + destTitle + pit + lateStr;
            lines.push(tripLine);
    ...
    // STRICT CULLING: Reserve 1 line for battery to equal exactly 10 lines
    if (lines.length > 9) {
    ...
    function isSafeHaven(testCoordsStr, timeSecs) {
        ...
        if (baseData !== "none" && baseData.length > 3) {
            ...
            for (var k = 0; k < bases.length; k++) {
                ...
                if (timeSecs * 1000 >= parseFloat(bp[0]) && timeSecs * 1000 <= parseFloat(bp[1])) {
    ...
    if (total_legs > 0) {
        ...
        if (bLabels.length < 3 && (nDep - now_sec) <= 7200) {
        ...
        if (baseData !== "none" && baseData.length > 3) { 
            ...
            for (var j = 0; j < bases.length; j++) { 
                ...
                if ((now_sec * 1000) >= parseFloat(bParts[0]) && (now_sec * 1000) <= parseFloat(bParts[1])) { 
    ...
    // ==========================================
    // SECTION 4: PUSH UI & BATTERY STATES TO TASKER
    // ==========================================
    ...
    // ==========================================
    // SECTION 5: SCHEDULE NEXT REFRESH
    // ==========================================
    var refreshTriggers = [];
