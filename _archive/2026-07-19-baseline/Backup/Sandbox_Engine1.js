...
// [V15.6] Stripped redundant Ad-Hoc guessing. Defers to UI Stop Logger.
// ==========================================
...
try {
    ...
    if (idx > master.length) { 
        ...
        if (incomingStatus === "Updating") {
            ...
            if (uLat !== 0 && uLng !== 0) {
                ...
                if (isAtMeeting) {
                    ...
                    let activeLeg = oldItin[0]; let leaveSec = 0; let legMode = "WALK";

                    if (activeLeg) { leaveSec = parseFloat(activeLeg.time || activeLeg.apiTimeUnix || activeLeg.start || 0); legMode = (activeLeg.mode || "WALK").toUpperCase().trim(); }
                    
                    if (leaveSec > 0 && nowSec >= leaveSec && nowSec < (leaveSec + 900)) {
        ...
        for (let i = idx; i <= master.length; i++) {
            ...
            if (i === idx) {
                isPrevBase = currentlyAtBase;
            } else {
                let pEv = master[i - 2];
                if (pEv && pEv.id && pEv.id.indexOf("_IN") !== -1) isPrevBase = true;
            }
            ...
            if (ev.isDropin) {
                ...
                if (nextStrict) {
                    ...
                    let distToNext = getDist(parseFloat(sLocP[0]), parseFloat(sLocP[1]), parseFloat(nC[0]), parseFloat(nC[1]));
                    let baseDriveSecs = Math.round(distToNext / 13.0);
                    let windowEnd = state.time + baseDriveSecs + (ev.duration || 1800) + 1800;
                    
                    if (evStart <= windowEnd) {
                        evStartTarget = 2000000000; 
                        isAttached = true;
                    }
                }
                
                if (!isAttached && state.time > (evStart + 7200)) {
            ...
            if (!ev.isDropin && (distToEventDirect < arrivalSkipRadius || (isMeetingLatched && distToEventDirect < 1000)) && (evStart - state.time) < 10800 && state.time < evDeadline) {
                ...
                state.loc = evCoords; skipIdx = i + 1; continue;
            }
            ...
            if ((testTargetTime > doorTarget || lookAheadLate > 0) && engineLateMins > 0 && !isBypassed) { 
                ...
                if (!isBypassed) {
                    ...
                    let tArr = simulateChainArrival(i, i, stateHistory[i], "TRANSIT", null); let tLate = Math.max(0, Math.ceil((tArr - doorTarget) / 60));
                    if (routeToEv.mode === "DRIVE") {
                        rawOptions.push({ label: "Park & take Transit" + getFlag("TRANSIT"), payload: "TRANSIT|" + evId + "|" + routeSig, late: tLate });
                        let wArr = simulateChainArrival(i, i, stateHistory[i], "WALK", null); let wLate = Math.max(0, Math.ceil((wArr - doorTarget) / 60));
                        ...
                        let lArr = simulateChainArrival(i, i, stateHistory[i], "LIFT", null); let lLate = Math.max(0, Math.ceil((lArr - doorTarget) / 60));
                        rawOptions.push({ label: "Take Lift instead" + getFlag("LIFT"), payload: "LIFT|" + evId + "|" + routeSig, late: lLate });
                        let dArr = simulateChainArrival(i, i, stateHistory[i], "DRIVE", null); let dLate = Math.max(0, Math.ceil((dArr - doorTarget) / 60));
                    ...
                    for (let k = i - 1; k >= idx; k--) {
                        ...
                        if (routeToEv.mode !== "DRIVE") {
                            let cArrD = simulateChainArrival(k, i, stateHistory[k], "DRIVE", null); let cLateD = Math.max(0, Math.ceil((cArrD - doorTarget) / 60));
                            rawOptions.push({ label: "Get Car before " + pTitle + " & Drive", payload: "DRIVE_CHAIN|" + pId + "~" + tailId, late: cLateD });
                        }
                        let sArr = simulateChainArrival(k, i, stateHistory[k], routeToEv.mode, pId); let sLate = Math.max(0, Math.ceil((sArr - doorTarget) / 60));
                        
                        if (!pIsEssential) {
                            rawOptions.push({ label: "Skip '" + pTitle + "' entirely", payload: "SKIP_EVENT|" + pId, late: sLate });
                        }
                        ...
                        let deadDrop = pEnd - ((engineLateMins - sLate) * 60);
                        let canTrim = true;
                        if (pIsEssential && !isEssential) canTrim = false;

                        if (canTrim && deadDrop < pEnd && deadDrop > evalStart) {
                            let simTrim = simulateScenario(i, { [k]: { trimEnd: deadDrop } });
                            if (simTrim.target && simTrim.maxSpill <= engineLateMins && simTrim.target.late < engineLateMins) {
            ...
            let apiTimeType = "DEPART"; let apiTimeUnix = state.time;
            if (isPrevBase) {
                if (!isDepart) {
                    apiTimeType = "DEPART"; 
                    apiTimeUnix = ev.isDropin ? Math.max(state.time, openUnix - routeTimeSecs) : Math.max(state.time, evStartTarget - routeTimeSecs); 
                } else { 
                    apiTimeType = "DEPART"; 
                    apiTimeUnix = ev.isDropin ? Math.max(state.time, openUnix) : Math.max(state.time, evStart); 
                }
            } else {
                apiTimeType = "DEPART";
            ...
            if (i === 1 && idx === 1 && (resolvedStatus.indexOf("Driving") !== -1 || resolvedStatus.indexOf("Walking") !== -1 || resolvedStatus.indexOf("Public Transport") !== -1 || resolvedStatus.indexOf("Lift") !== -1)) {
            ...
            let displayTime = ev.isDropin ? Math.max(state.time, openUnix) : evStart;
            let safeDesc = encodeURIComponent(evDesc);
            
            queue.push("EVENT|" + evTitle + "|" + evCoords + "|" + routeToEv.mode + "|" + displayTime + "|" + trueDepartureTime + "|" + pitstopState + "|" + apiTimeType + "|" + apiTimeUnix + "|" + evId + "|" + evLoc + "|" + engineLateMins + "|" + currentLegStable + "|none|" + safeDesc + "|" + adHocObj.arr.join(","));
